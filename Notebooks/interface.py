...
# Titre principal
st.markdown('<h1 class="main-header">📊 Interface de Prévision des Indicateurs Économiques du Maroc</h1>',
            unsafe_allow_html=True)
st.markdown("---")
...
def get_model_mapping():
    ...
    default_mapping = {
        "IDE(USD)": "LSTM-CNN",
        "REER(2010 = 100)": "GRU",
        "brent_oil_prices(USD/barrel)": "TCN",
        "crude_oil_prices(USD/barrel)": "GRU",
        "daily_natural_gas_prices(USD/MMBtu)": "GRU",
        ...
        "Chômage, total (% de la population)": "GRU",
        "Chômage, total jeune entre 15-24 (% de la population)": "RNN",
...
def main():
    ...
    if missing_models:
        st.sidebar.warning(f"⚠️ Modèles manquants: {', '.join(missing_models)}")

    # Sélection des indicateurs
    st.sidebar.subheader("📈 Indicateurs à analyser")

    # Grouper les indicateurs par catégorie
    economic_indicators = [col for col in num_cols if
                           any(term in col.lower() for term in ['ide', 'reer', 'inflation', 'chomage'])]
    commodity_indicators = [col for col in num_cols if
                            any(term in col.lower() for term in ['oil', 'gold', 'wheat', 'gas'])]
    social_indicators = [col for col in num_cols if
                         any(term in col.lower() for term in ['demographie', 'pauvrete', 'analphabetisme'])]
    trade_indicators = [col for col in num_cols if any(term in col.lower() for term in ['export', 'import', 'tourism'])]
    other_indicators = [col for col in num_cols if
                        col not in economic_indicators + commodity_indicators + social_indicators + trade_indicators]
    ...
    if st.button(" Générer les prévisions", type="primary", use_container_width=True):
        with st.spinner(" Génération des prévisions en cours..."):
            ...
            forecast_dates = pd.date_range(start=forecast_start, periods=forecast_periods, freq='MS')

            results = {}

            for indicator in selected_indicators:
                st.subheader(f" Prévisions pour {indicator}")

                # Obtenir les données de l'indicateur
                indicator_data = df[indicator].dropna()

                if len(indicator_data) == 0:
                    st.error(f"❌ Pas de données pour {indicator}")
                    continue
                ...
                if TF_AVAILABLE and SKLEARN_AVAILABLE:
                    ...
                    if model_available:
                        # TODO: Charger et utiliser le vrai modèle
                        # Pour l'instant, utiliser la méthode simple
                        predictions = create_simple_forecast(indicator_data, forecast_periods)
                        method_used = f"{recommended_model} (simulé)"
                    else:
                        predictions = create_simple_forecast(indicator_data, forecast_periods)
                        method_used = "Extrapolation linéaire"
                else:
                    predictions = create_simple_forecast(indicator_data, forecast_periods)
                    method_used = "Extrapolation linéaire"
                ...
                recent_data = indicator_data.tail(min(100, len(indicator_data)))
                fig.add_trace(go.Scatter(
                    x=recent_data.index,
                    y=recent_data.values,
                    mode='lines',
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y=predictions,
                    mode='lines+markers',
                    name=f'Prévisions ({method_used})',
                    line=dict(color='#ff7f0e', width=2, dash='dash'),
                    marker=dict(size=6),
                    hovertemplate='Date: %{x}<br>Prévision: %{y:,.2f}<extra></extra>'
                ))

                # Ligne de séparation
                fig.add_vline(
                    x=pd.to_datetime(forecast_start),
                    line_dash="dot",
                    line_color="gray",
                    annotation_text="Début des prévisions"
                )

                fig.update_layout(
                    title=f"Prévisions - {indicator}",
                    xaxis_title="Date",
                    yaxis_title=indicator,
                    hovermode='x unified',
                    height=500,
                ...
                if show_statistics:
                    col1, col2, col3, col4 = st.columns(4)
                    with col1:
                        st.metric(" Moyenne prévue", f"{predictions.mean():,.2f}")
                    with col2:
                        st.metric(" Valeur min", f"{predictions.min():,.2f}")
                    with col3:
                        st.metric(" Valeur max", f"{predictions.max():,.2f}")
                    with col4:
                ...
                with st.expander("📋 Voir les valeurs détaillées"):
                    forecast_df = pd.DataFrame({
                        'Date': forecast_dates,
                        f'Prévision_{indicator}': predictions,
                        'Méthode': method_used
                    })
                    st.dataframe(forecast_df, use_container_width=True)

            # Analyse comparative si plusieurs indicateurs
            if len(results) > 1:
                st.header(" Analyse Comparative des Prévisions")

                # Graphique comparatif
                fig = go.Figure()
                colors = px.colors.qualitative.Set1

                for i, (indicator, data) in enumerate(results.items()):
                    # Normaliser pour la comparaison
                    preds = data['predictions']
                    if preds.max() != preds.min():
                        normalized_preds = (preds - preds.min()) / (preds.max() - preds.min())
                    else:
                        normalized_preds = np.ones_like(preds) * 0.5
                ...
                fig.update_layout(
                    title="Comparaison des prévisions (données normalisées)",
                    xaxis_title="Date",
                    yaxis_title="Valeurs normalisées (0-1)",
                    height=500,
                    showlegend=True
                )
